...
import { RecipeAssistantAI } from '@/components/ai/recipe-assistant-ai';
...
import type { Product, Recipe, RecipeIngredientItem, ExpenseFixedCategory, Employee, ProductionLogEntry } from '@/lib/types/db-types';
...
export default function RecipesPage() {
  ...
  const handleOpenEditDialog = (recipe: Recipe) => {
    ...
    setEditOutputUnit(recipe.outputUnit || 'l');
  ...
  const renderDialogContent = () => {
    ...
    return (
      <div className="grid gap-4 py-4">
        ...
        {!isEditing && !isNonDispatchableModeDialog && !isResoldProduct && (
          <div className="mb-4">
            <RecipeAssistantAI
              recipeName={recipeName}
              category={category || 'Producto Final'}
              onApplySuggestions={(suggestedIngs, yieldVal) => {
                setNewIngredients(suggestedIngs);
                setNewExpectedYield(yieldVal.toString());
              }}
              disabled={!recipeName || recipeName.length < 3}
            />
          </div>
        )}
